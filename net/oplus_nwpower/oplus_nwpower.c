...
static void oplus_ipa_schedule_work() {
...
static void tcp_output_hook_work_callback(struct work_struct *work) {
	...
	if (tcp_output_list.is_ipv6) {
		...
		snprintf(bts_net_wakeup_buffer, BTS_BUFFER_SIZE, "ou%d", tcp_output_list.uid);
	} else {
...
static void tcp_input_hook_work_callback(struct work_struct *work) {
	...
	if (tcp_input_list.is_ipv6) {
		...
		snprintf(bts_net_wakeup_buffer, BTS_BUFFER_SIZE, "iu%d", tcp_input_list.uid);
	} else {
