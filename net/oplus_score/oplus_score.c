...
#define LOG_INTERVAL_THRESHOLD (3 * HZ)
...
static u32 oplus_score_report_log_lasttime = 0;
static u32 oplus_score_up_detail_log_lasttime = 0;
static u32 oplus_score_down_detail_log_lasttime = 0;
...
void oplus_score_calc_and_report(void)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		if (oplus_score_debug || (uplink_smooth_score > 0 && uplink_smooth_score <= WORST_BASE_SCORE)) {
			...
			u32 interval = 0;
			nowtime = jiffies;
			interval = nowtime - oplus_score_up_detail_log_lasttime;
			if (oplus_score_up_detail_log_lasttime == 0 || interval < 0 || (oplus_score_up_detail_log_lasttime != 0 && interval >= LOG_INTERVAL_THRESHOLD)) {
				printk("[oplus_score]:up_score:link=%u,if=%s,up_pack=%u,up_retran=%u,up_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u,nodata=%u,"
					...
					nowtime, oplus_score_up_detail_log_lasttime, interval, LOG_INTERVAL_THRESHOLD);
				oplus_score_up_detail_log_lasttime = nowtime;
			}
		}
		...
		if (oplus_score_debug || (downlink_smooth_score > 0 && downlink_smooth_score <= WORST_BASE_SCORE)) {
			...
			u32 interval = 0;
			nowtime = jiffies;
			interval = nowtime - oplus_score_down_detail_log_lasttime;
			if (oplus_score_down_detail_log_lasttime == 0 || interval < 0 || (oplus_score_down_detail_log_lasttime != 0 && interval >= LOG_INTERVAL_THRESHOLD)) {
				printk("[oplus_score]:down_score:link=%u,if=%s,down_pack=%u,down_retran=%u,rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u,nodata=%u,"
					"nowtime=%u,lasttime=%u,inv=%u,thrs=%u\n",
					...
					nowtime, oplus_score_down_detail_log_lasttime, interval, LOG_INTERVAL_THRESHOLD);
				oplus_score_down_detail_log_lasttime = nowtime;
			}
		}
		...
		if (uplink_report || downlink_report) {
			...
			if (oplus_score_debug || (downlink_smooth_score <= WORST_BASE_SCORE) || (uplink_smooth_score <= WORST_BASE_SCORE)) {
				...
				u32 interval = 0;
				nowtime = jiffies;
				interval = nowtime - oplus_score_report_log_lasttime;
				if (oplus_score_report_log_lasttime == 0 || interval < 0 || (oplus_score_report_log_lasttime != 0 && interval >= LOG_INTERVAL_THRESHOLD)) {
					printk("[oplus_score]:report_score1:link=%u,if=%s,up_score=%d,down_score=%d,uid=%u,ul_p=%d,dl_p=%d,nowtime=%u,lasttime=%u,inv=%u,thrs=%u\n",
						uplink_index, ifname, link_score_msg.uplink_score, link_score_msg.downlink_score,
						oplus_score_foreground_uid[0], uplink_report, downlink_report, nowtime, oplus_score_report_log_lasttime, interval, LOG_INTERVAL_THRESHOLD);
					oplus_score_report_log_lasttime = nowtime;
				}
			}
		}
