...
static int oppo_game_latency_info = 0;
/* OPLUS_SLA_GAME_LATENCY */
...
static void init_dual_wifi_weight(void)
{
	...
	if (INIT_ACTIVE_TYPE != dual_wifi_active_type &&
		...
		if (oppo_sla_info[MAIN_WLAN].wlan_score == WLAN_NETWORK_INVALID) {
			oppo_sla_info[MAIN_WLAN].weight = 0;
			oppo_sla_info[SECOND_WLAN].weight = 100;
		} else {
...
static void game_app_switch_network(u32 game_type)
{
	...
	if (cell_quality_good &&
	   ...
	   oppo_game_latency_info >= max_latency) &&
		...
		if (!game_uid[index].switch_time ||
			...
			printk("oppo_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,changing to cellular...\n",
				uid, game_rtt, wlan_bad, game_rx_bad, oppo_game_latency_info);
			oppo_game_latency_info = 0;
			/* OPLUS_SLA_GAME_LATENCY */
			return;
		}
	}
	...
	if (!wlan_bad &&
	   game_uid[index].mark == CELL_MARK &&
	   (game_rx_bad || oppo_game_latency_info >= max_latency)) {
	...
	if (is_game_rtt_skb(ct, skb, false)) {
		...
		if (ct->oppo_game_timestamp && time_now > ct->oppo_game_timestamp) {
			...
			if (sla_switch_enable && cell_quality_good) {
				if (oppo_sla_enable && sla_work_mode == SLA_MODE_DUAL_WIFI) {
					...
					if (game_rx_bad || game_rtt >= (MAX_GAME_RTT - 80)) {
						...
						printk("oppo_sla_netlink: dualsta Mode, game app send enable sla to user\n");
					}
				} else if (!enable_cell_to_user && !oppo_sla_enable) {
					//Normal Mode, enable sla
					if (game_rx_bad || game_rtt >= (MAX_GAME_RTT / 2)) {
						send_enable_to_framework(SLA_MODE_WIFI_CELL, INIT_ACTIVE_TYPE);
						printk("oppo_sla_netlink: Normal Mode, game app send enable sla to user\n");
					}
				}
			}
...
static void oppo_sla_set_game_latency(struct nlmsghdr *nlh)
{
	u32 *data = (u32 *)NLMSG_DATA(nlh);
	oppo_game_latency_info = data[0];
	printk("oppo_sla_set_game_latency: oppo_game_latency_info [%d]\n", oppo_game_latency_info);
}
/* OPLUS_SLA_GAME_LATENCY */
