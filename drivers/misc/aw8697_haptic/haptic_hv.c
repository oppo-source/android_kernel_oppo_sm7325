...
#define HAPTIC_HV_DRIVER_VERSION	"v0.0.0.4"
			  ...
			  char *buf)
{
	...
	aw_haptic->index = seq;
	count += snprintf(buf, PAGE_SIZE, "index = %d\n",
			  aw_haptic->index);
	return count;
}
...
static void motor_old_test_work(struct work_struct *work)
{
	...
	if (aw_haptic->motor_old_test_mode == MOTOR_OLD_TEST_TRANSIENT) {
		...
		aw_haptic->gain = 0x80;
		aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
		aw_haptic->func->set_bst_vol(aw_haptic, AW_HAPTIC_HIGH_LEVEL_REG_VAL);
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->func->protect_config(aw_haptic, 0x00, 0x00);
