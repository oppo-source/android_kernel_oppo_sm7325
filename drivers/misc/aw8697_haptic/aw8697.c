...
static void aw_vib_vmax_set(struct aw8697 *aw8697);
         ...
         unsigned char reg_addr, unsigned int mask, unsigned char reg_data)
{
    ...
    if (temp == reg_val)
        return 0;
    aw8697_i2c_write(aw8697, reg_addr, temp);
...
static int aw8697_haptic_play_mode(struct aw8697 *aw8697, unsigned char play_mode)
{
    ...
    if(aw8697->play_mode == play_mode) {
        pr_err("%s two time set playmode = %d\n", __func__, play_mode);
        return 0;
    }
    switch(play_mode) {
...
{
    unsigned long int interval_us;
    //unsigned char reg_val = 0;
    pr_err("%s enter,flag = %d\n", __func__,flag);


    if(flag) {
        /*pr_debug("%s enter,start vib\n", __func__);*/
        aw8697->vib_stop_flag = false;
        aw8697_i2c_write_bits(aw8697, AW8697_REG_GO,
            AW8697_BIT_GO_MASK, AW8697_BIT_GO_ENABLE);
        aw8697->pre_enter_time = ktime_get();
    } else {
        aw8697->current_time = ktime_get();
        interval_us = ktime_to_us(ktime_sub(aw8697->current_time, aw8697->pre_enter_time));
        if (interval_us < 2000) {
            pr_err("%s: aw8697->interval_us=%d < 2000\n", __func__, interval_us);
            mdelay(2);
        }
        aw8697_i2c_write_bits(aw8697, AW8697_REG_GO,
...
static int aw8697_haptic_stop_delay(struct aw8697 *aw8697)
{
    ...
    while(cnt--) {
        ...
        if((reg_val&0x0f) == 0x00) {
            pr_err("%s enter,already stop\n", __func__);
            aw8697->vib_stop_flag = true;
            return 0;
        }
...
static int aw8697_haptic_ram_vbat_comp(struct aw8697 *aw8697, bool flag)
{
    ...
    if(flag) {
        if(aw8697->ram_vbat_comp == AW8697_HAPTIC_RAM_VBAT_COMP_ENABLE) {
            ...
            temp_gain = aw8697->vmax_gain * AW8697_VBAT_REFER / aw8697->vbat;
            ...
            aw8697_haptic_set_gain(aw8697, aw8697->vmax_gain);
        }
    } else {
        aw8697_haptic_set_gain(aw8697, aw8697->vmax_gain);
    }
    pr_debug("%s: aw8697->vmax_gain = 0x%x, temp_gain = 0x%x, aw8697->ram_vbat_comp=%d.\n",
        __func__, aw8697->vmax_gain, temp_gain, aw8697->ram_vbat_comp);
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
    ...
    if (aw8697->rtp_file_num >= (sizeof(aw8697_rtp_name) / AW8697_RTP_NAME_MAX))
        return;
...
#if 0 //delete by aw, this has no use.
...
static int aw8697_haptic_init(struct aw8697 *aw8697)
{
    ...
    aw8697->vmax_gain = reg_val & 0xFF;
    ret = aw8697_i2c_read(aw8697, AW8697_REG_BSTDBG4, &reg_val);
#ifdef OPLUS_FEATURE_CHG_BASIC
    aw8697->vmax_vmax = AW8697_HAPTIC_HIGH_LEVEL_REG_VAL;
    aw8697->vmax_level = AW8697_HAPTIC_VMAX_LEVEL_MAX;
    aw8697->play_mode = AW8697_HAPTIC_UNKNOW_MODE;
#else
    aw8697->vmax_vmax = (reg_val>>1)&0x1F;
#endif
                ...
                enum led_brightness level)
{
    ...
    if (!queue_work(system_highpri_wq, &aw8697->short_vibrator_work)) {
        pr_err("%s: need_set_brightness error not ok \n", __FUNCTION__ );
    } else {
        pr_debug("%s: need_set_brightness queue success\n", __FUNCTION__);
    }
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    aw8697->state = val;
	if(val == 0 && aw8697->vib_stop_flag)
		return count;
#ifdef OPLUS_FEATURE_CHG_BASIC
    cancel_work_sync(&aw8697->vibrator_work);
    //schedule_work(&aw8697->vibrator_work);
    if(queue_work(system_highpri_wq, &aw8697->vibrator_work)) {
        pr_err("%s: vibrator_work success val = %d\n", __FUNCTION__, val);
    } else {
        pr_err("%s: vibrator_work not ok val = %d\n", __FUNCTION__, val);
    }
#endif
    pr_err("%s: return\n", __FUNCTION__);
    return count;
...
static int aw8697_RTP_is_going_on_without_lock(struct aw8697 *aw8697)
{
    unsigned char reg_val = 0;
    unsigned char rtp_state = 0;
...
int aw8697_convert_level_to_vmax(struct aw8697 *aw8697, int val)
{
    ...
    for(i = 0; i < ARRAY_SIZE(vmax_map); i++) {
        if(val == vmax_map[i].level) {
            aw8697->vmax_vmax = vmax_map[i].vmax;
            aw8697->vmax_gain = vmax_map[i].gain;
            break;
        }
    }
    if(i == ARRAY_SIZE(vmax_map)) {
        aw8697->vmax_vmax = vmax_map[i - 1].vmax;
        aw8697->vmax_gain = vmax_map[i - 1].gain;
    }
...
{
#ifdef OPLUS_FEATURE_CHG_BASIC
        if (aw8697->vmax_level <= 255) {
            aw8697->vmax_gain = (aw8697->vmax_level * AW8697_HAPTIC_RAM_VBAT_COMP_GAIN) / 255;
        } else if (aw8697->vmax_level <= AW8697_HAPTIC_VMAX_LEVEL_MAX) {
            aw8697_convert_level_to_vmax(aw8697, aw8697->vmax_level);
        } else {
            aw8697->vmax_vmax = AW8697_HAPTIC_HIGH_LEVEL_REG_VAL;
            aw8697->vmax_gain = AW8697_HAPTIC_RAM_VBAT_COMP_GAIN;
        }

        if (aw8697->vmax_level == 2550) {    // for old test only
            aw8697->vmax_gain = AW8697_HAPTIC_RAM_VBAT_COMP_GAIN;
        }
        ...
        aw8697_haptic_set_gain(aw8697, aw8697->vmax_gain);
        aw8697_haptic_set_bst_vol(aw8697, aw8697->vmax_vmax);
#else
        aw8697_haptic_set_bst_vol(aw8697, aw8697->vmax_vmax);
#endif
        pr_debug("%s:aw8697->vmax_gain[0x%x], aw8697->vmax_vmax[0x%x] end\n", __FUNCTION__, aw8697->vmax_gain, aw8697->vmax_vmax);
}
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    rc = kstrtouint(buf, 0, &val);
    if (rc != 0) {
        pr_err("%s: kstrtouint error\n", __FUNCTION__);
        return count;
    }
    pr_err("%s: value=%d\n", __FUNCTION__, val);

    aw8697->vmax_level = val;

    rtp_is_going_on = aw8697_RTP_is_going_on_without_lock(aw8697);
    if (rtp_is_going_on) {
        mutex_lock(&aw8697->lock);
        aw_vib_vmax_set(aw8697);
        mutex_unlock(&aw8697->lock);
        ...
        const char *buf, size_t count)
{
    ...
    if (rc < 0)
        return rc;
    pr_err("%s: val [%d] \n", __func__, val);
    if(!pre_val && (val == pre_val)) {
        pr_debug("%s: val two zero \n", __func__, val);
        return count;
    }
    ...
    if (!val) {
        ...
        aw8697_op_clean_status(aw8697);
        if(!queue_work(system_unbound_wq, &aw8697->rtp_work))
            pr_err("%s:error not queue canot stop\n", __FUNCTION__);
        return count;
    }
    ...
    aw8697->rtp_file_num = val;
    if(!queue_work(system_unbound_wq, &aw8697->rtp_work))
        pr_err("%s:error not queue canot start\n", __FUNCTION__);
    pr_debug("%s: return\n", __func__);
    return count;
}
...
static void motor_old_test_work(struct work_struct *work)
{
    ...
    pr_err("%s: motor_old_test_mode = %d. aw8697->vmax_gain[0x%02x]\n", __func__, aw8697->motor_old_test_mode, aw8697->vmax_gain);

    if (aw8697->motor_old_test_mode == MOTOR_OLD_TEST_TRANSIENT) {
        mutex_lock(&aw8697->lock);

        aw8697_haptic_stop(aw8697);
        aw8697->vmax_gain = 0x80;
        aw8697_haptic_set_gain(aw8697, aw8697->vmax_gain);
                ...
                const char *buf, size_t count)
{
    ...
    if(1 == sscanf(buf, "%d", &databuf[0])) {
        index = (unsigned char)databuf[0];
            aw8697->seq[0] = index;
    }
    pr_err("%s: return index = %d\n", __FUNCTION__, databuf[0]);
    return count;
}
...
static void aw8697_short_vibrator_work_routine(struct work_struct *work)
{
    struct aw8697 *aw8697 = container_of(work, struct aw8697, short_vibrator_work);

    if (aw8697_haptic_juge_RTP_is_going_on(aw8697))
    {
        pr_err("%s aw8697_haptic_juge_RTP_is_going_on exit\n", __func__);
        return;
    }
    mutex_lock(&aw8697->lock);
    aw8697_haptic_stop(aw8697);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
    pr_debug("%s enter, state[%d], activate_mode[%d], ram_vbat_comp[%d]\n",
                __func__, aw8697->state, aw8697->activate_mode, aw8697->ram_vbat_comp);
#endif
    mutex_lock(&aw8697->lock);
    aw8697_haptic_stop(aw8697);
    if(aw8697->state) {
        if (aw8697->vmax_gain >= AW8697_HAPTIC_RAM_VBAT_COMP_GAIN) {
            aw8697->vmax_gain = AW8697_HAPTIC_RAM_VBAT_COMP_GAIN;
        }
        ...
        hrtimer_start(&aw8697->timer,
                  ktime_set(aw8697->duration / 1000, (aw8697->duration % 1000) * 1000000),
                  HRTIMER_MODE_REL);
    }
    mutex_unlock(&aw8697->lock);
...
static int aw8697_vibrator_init(struct aw8697 *aw8697)
{
    ...
    INIT_WORK(&aw8697->short_vibrator_work, aw8697_short_vibrator_work_routine);
