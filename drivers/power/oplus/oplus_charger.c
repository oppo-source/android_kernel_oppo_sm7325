...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->dual_charger_support || (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY
		|| oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY)) {
		if (chip->charger_volt > 7500) {
			chip->pd_chging = true;
		} else {
	...
	case CHARGER_SUBTYPE_FASTCHG_VOOC:
		if (chip->chg_ctrl_by_vooc == true) {
			if (c_level_index <= 0) {
				...
				chip->limits.input_current_vooc_ma_high =
						val <  cool_down_current_limit_vooc[vooc_index_temp - 1] ? val : cool_down_current_limit_vooc[vooc_index_temp - 1];
	...
	case CHARGER_SUBTYPE_QC:
		if (c_level_index <= 0) {
			...
			if(subtype == CHARGER_SUBTYPE_PD)
				chip->limits.input_current_cool_down_ma =
						choose_little_current(chip->limits.input_current_cool_down_ma, chip->limits.default_pd_input_current_charger_ma);
			else
				chip->limits.input_current_cool_down_ma =
						choose_little_current(chip->limits.input_current_cool_down_ma, chip->limits.default_qc_input_current_charger_ma);
...
/*		charger_xlog_printk(CHG_LOG_CRTI, "batt_temp=%d,sub_batt_temp=%d,shell_temp=%d,chging_temp=%d\n",
					batt_temp, sub_batt_temp, shell_temp, chging_temp);*/
	} else {
		if (oplus_chg_get_voocphy_support() == NO_VOOCPHY && oplus_vooc_get_fastchg_started() == true) {
			batt_temp = oplus_gauge_get_prev_batt_temperature();
		} else {
