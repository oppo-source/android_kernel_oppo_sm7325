...
static void oplus_oem_lcm_en_check_work(struct work_struct *work)
{
	...
	vph_track_high = (chip->sw_full ? 1 : 0);
	...
	if (vph_track_high)
		bcdev->oem_misc_ctl_data |= OEM_MISC_CTL_DATA_PAIR(OEM_MISC_CTL_CMD_LCM_25K, vph_track_high);
	oplus_oem_misc_ctl();
	last_enable = enable;
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (chip->wls_status_keep != WLS_SK_NULL) {
			...
			if (pre_wls_online && pval->intval == 0) {
				...
				pval->intval = 1;
				schedule_delayed_work(&bcdev->status_keep_clean_work, msecs_to_jiffies(1000));
			} else {
				pre_wls_online = pval->intval;
				if (bcdev->status_wake_lock_on) {
...
static void battery_chg_shutdown(struct platform_device *pdev)
{
	...
	if (!is_ext_chg_ops() && g_oplus_chip) {
		bcdev->oem_misc_ctl_data = 0;
