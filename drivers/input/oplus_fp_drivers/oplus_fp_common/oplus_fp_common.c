...
#ifndef FINGERPRINT_GKI_DISABLE
...
static int fp_gpio_parse_parent_dts(struct fp_data *fp_data)
{
	...
	for (fp_id_index = 0; fp_id_index < fp_data->fp_id_amount; fp_id_index++) {
		...
		snprintf(fp_gpio_current_node, FP_ID_MAX_LENGTH - 1, "%s%d",
			 FP_GPIO_PREFIX_NODE, fp_id_index);
		dev_info(fp_data->dev, "fp_gpio_current_node: %s\n", fp_gpio_current_node);
		fp_data->gpio_index[fp_id_index] = of_get_named_gpio(np, fp_gpio_current_node,
						   0);
...
static int opticalfp_touch_event_notify(struct notifier_block *self,
...
static int fp_gpio_parse_child_dts(struct fp_data *fp_data)
{
	...
	for_each_available_child_of_node(np, child) {
		child_amount = of_property_count_elems_of_size(child, FP_ID_VALUE_NODE,
				sizeof(u32));
		...
		for (child_node_index = 0; child_node_index < child_amount;
				child_node_index++) {
			ret = of_property_read_u32_index(child, FP_ID_VALUE_NODE, child_node_index,
...
void read_lcd_type_proc_data(void)
{
	...
	else if (strstr(saved_command_line, "tianma_ili"))
		strncpy(lcd_manu, "tm2", FP_ID_MAX_LENGTH - 1);
