...
int gf_parse_pwr_list(struct gf_dev *gf_dev)
{
	...
	for_each_available_child_of_node(np, child) {
		...
		ret = of_property_read_u32(child, FP_POWER_NODE,
		...
		case FP_POWER_MODE_LDO:
			...
			ret = of_property_read_u32_index(np, node_name, LDO_VMAX_INDEX,
			...
			ret = of_property_read_u32_index(np, node_name, LDO_VMIN_INDEX,
			...
			ret = of_property_read_u32_index(np, node_name, LDO_UA_INDEX,
		...
		case FP_POWER_MODE_GPIO:
			...
			/* get gpio by name */
			gf_dev->pwr_list[child_node_index].pwr_gpio = of_get_named_gpio(np, node_name,
					0);
			...
			ret = of_property_read_u32(child, FP_POWERON_LEVEL_NODE,
...
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->is_optical == true) {
		gf_dev->pstate_spi_6mA  = pinctrl_lookup_state(gf_dev->pinctrl,
					  "gf_spi_drive_6mA");
		...
		gf_dev->pstate_irq_no_pull = pinctrl_lookup_state(gf_dev->pinctrl,
					     "goodix_irq_no_pull");
	...
	/*get reset resource*/
	gf_dev->reset_gpio = of_get_named_gpio(pdev->dev.of_node, "goodix,gpio_reset",
					       0);
