...
static int init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	ts->is_spiport_ic           = of_property_read_bool(np, "spi_port_support");
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
	...
	if ((!ts->is_noflash_ic) && (!ts->is_spiport_ic)) {
...
static void tp_resume(struct device *dev)
{
	...
	TP_INFO(ts->tp_index, "%s start.\n", __func__);
	tmp_prj_id = get_project();
	TP_INFO(ts->tp_index, "the project id is %d\n", tmp_prj_id);
	if (tmp_prj_id == 21143) {
	/*add for temp testing*/
	ts->force_update = 1;
