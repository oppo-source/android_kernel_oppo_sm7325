 ...
 * SN220_CHIPID = "0xc1" PN560_CHIPID = "0x00"
 */
	/*debug to delete*/
	pr_err("%s : nfc_dev->nqx_info.info.chip_type:%02x\n", __func__, nfc_dev->nqx_info.info.chip_type);


	if ((nfc_dev->nqx_info.info.chip_type != 0xc1) &&
		(nfc_dev->nqx_info.info.chip_type != 0xca) &&
		(nfc_dev->nqx_info.info.chip_type != 0x00)) {
		int retrycount = 0;
		char wakeup_cmd[1] = {0};
		while (++retrycount < 6) {
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		//not returning failure here as dwl gpio is a optional gpio for sn220/pn560
	}
